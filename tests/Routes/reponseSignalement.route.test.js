...
describe('ReponseSignalement API', () => {
    ...
    it('POST /api/reponses-signalement - doit créer une réponse de signalement', async () => {
        ...
        const signalement = await Signalement.create({ titre: 'Problème de stage', description: 'Pas de tuteur', etudiant: etudiant._id });

        const res = await request(app)
            .post('/api/reponses-signalement')
            .set('Authorization', `Bearer ${token}`)
            .send({ signalement: signalement._id, numero: 1 });
    ...
    it('GET /api/reponses-signalement - doit retourner une liste de réponses', async () => {
        ...
        const etudiant = await Etudiant.create({ utilisateur: utilisateur._id, dateDebut: new Date('2024-01-01') });
        const signalement = await Signalement.create({ titre: 'Problème de cours', description: 'Pas de prof', etudiant: etudiant._id });
        await ReponseSignalement.create({ signalement: signalement._id, numero: 1 });

        const res = await request(app)
            .get('/api/reponses-signalement')
            .set('Authorization', `Bearer ${token}`);
    ...
    it('GET /api/reponses-signalement/:id - doit retourner une réponse spécifique', async () => {
        ...
        const etudiant = await Etudiant.create({ utilisateur: utilisateur._id, dateDebut: new Date('2024-01-01') });
        const signalement = await Signalement.create({ titre: 'Problème d\'évaluation', description: 'Pas de retour', etudiant: etudiant._id });
        const reponse = await ReponseSignalement.create({ signalement: signalement._id, numero: 2 });

        const res = await request(app)
            .get(`/api/reponses-signalement/${reponse._id}`)
            .set('Authorization', `Bearer ${token}`);
    ...
    it('PUT /api/reponses-signalement/:id - doit mettre à jour une réponse', async () => {
        ...
        const etudiant = await Etudiant.create({ utilisateur: utilisateur._id, dateDebut: new Date('2024-01-01') });
        const signalement = await Signalement.create({ titre: 'Autre problème', description: 'Autre soucis', etudiant: etudiant._id });
        const reponse = await ReponseSignalement.create({ signalement: signalement._id, numero: 3 });
    ...
    it('DELETE /api/reponses-signalement/:id - doit supprimer une réponse', async () => {
        ...
        const etudiant = await Etudiant.create({ utilisateur: utilisateur._id, dateDebut: new Date('2024-01-01') });
        const signalement = await Signalement.create({ titre: 'Encore un souci', description: 'Pas clair', etudiant: etudiant._id });
        const reponse = await ReponseSignalement.create({ signalement: signalement._id, numero: 5 });

        const res = await request(app)
            .delete(`/api/reponses-signalement/${reponse._id}`)
            .set('Authorization', `Bearer ${token}`);
